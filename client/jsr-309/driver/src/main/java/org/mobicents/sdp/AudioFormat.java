...
public class AudioFormat extends Format {
	...
	public AudioFormat(String encoding, double sampleRate, int sampleSizeInBits, int channels, int endian, int signed,
			int frameSizeInBits, double frameRate) {
		super(encoding);
		this.sampleRate = sampleRate;
		this.sampleSizeInBits = sampleSizeInBits;
		this.channels = channels;
	...
	/**
	 * Overrides default toString. Flag indicates if output should be equal to parsed string: dolbyac3, 2, 1, 2;
	 * 
	 * @param asInput
	 *            <ul>
	 *            <li> <b>true</b> - will return in the exact same way it has been fed: dolbyac3, 2, 1, 2 - this format
	 *            is defined in rtp media format
	 *            <li> <b>false</b> - will return in format like: dolbyac3, 2.0 Hz, 1-bit, Stereo, Unsigned, 6.0 frame
	 *            rate
	 *            </ul>
	 * @return
	 */
	...
	public boolean matches(Format format) {
		...
		final boolean result = FormatUtils.matches(this.sampleRate, oCast.sampleRate)
				&& FormatUtils.matches(this.sampleSizeInBits, oCast.sampleSizeInBits)
				&& FormatUtils.matches(this.channels, oCast.channels) && FormatUtils.matches(this.endian, oCast.endian)
